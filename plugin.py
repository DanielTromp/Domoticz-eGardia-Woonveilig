...
# Author: Daniel Tromp
#
# Based on code from: System67 (Carlo)
#
    ...
    def onStart(self):
        ...
        for Sensor in Sensors:
            ...
            if (int(SensorData[self._NrKey]) not in Devices):
                ...
                if (SensorData['type_f'] in self._TypeIR): 
...
def getsensorstate(prm_Sensor, prm_Version):
    if prm_Sensor is not None:
        ...
        if prm_Version in ["GATE-01", "GATE-02"]:
            ...
            if prm_Sensor["status"].upper() == "DOOR OPEN":
                # Return True when door is open or IR is triggered (todo) or len(prm_Sensor["status"]) >0:
                return False
            elif prm_Sensor["status"].upper() == "DOOR CLOSE":
#            else:
                # Return False when door is closed or IR is not triggered (todo) or len(prm_Sensor["status"]) <=0:
                return True
    else:
        return None
